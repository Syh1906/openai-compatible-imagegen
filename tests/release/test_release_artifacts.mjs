...
test("release workflow publishes an exact version-titled release from an annotated tag", async () => {
  ...
  const preflight = await readFile(releasePreflightWorkflow, "utf8");
  assert.match(workflow, /workflow_dispatch:/);
  assert.match(preflight, /os:\s*windows-latest/);
  assert.match(preflight, /os:\s*ubuntu-latest/);
  assert.match(preflight, /os:\s*macos-latest/);
  assert.match(preflight, /runs-on:\s*\$\{\{\s*matrix\.os\s*\}\}/);
  assert.match(workflow, /environment:\s*release/);
  assert.match(workflow, /permissions:\s*\n\s*contents:\s*write/);
  assert.match(workflow, /ref:\s*\$\{\{\s*steps\.preflight\.outputs\.source_sha\s*\}\}/);
  assert.match(workflow, /ref:\s*\$\{\{\s*needs\.validate_release\.outputs\.preflight_sha\s*\}\}/);
  assert.match(workflow, /fetch-depth:\s*0/);
  assert.equal(workflow.match(/uses:\s*actions\/setup-node@v4/g)?.length, 1);
  for (const command of [
    ...
    assert.ok(preflight.includes(command), command);
  }
  assert.match(workflow, /git cat-file -t/);
  assert.match(workflow, /plugin\.json[^\n]+\.version/);
  assert.match(preflight, /actions\/upload-artifact@v4/);
  ...
  assert.match(workflow, /run-id:\s*\$\{\{\s*inputs\.preflight_run_id\s*\}\}/);
  assert.match(workflow, /tagged_sha/);
  assert.match(preflight, /\^\[0-9a-fA-F\]\{40\}\$/);
  assert.match(preflight, /git rev-parse HEAD/);
  assert.match(preflight, /github\.event\.repository\.default_branch/);
  assert.match(preflight, /github\.ref_name/);
  assert.match(preflight, /needs\.resolve_source\.outputs\.source_sha/);
  assert.match(preflight, /\$\{\{\s*matrix\.artifact\s*\}\}-\$\{\{\s*github\.run_attempt\s*\}\}/);
  assert.match(preflight, /preflight-metadata-\$\{\{\s*github\.run_attempt\s*\}\}/);
  assert.match(preflight, /preflight-metadata\/preflight\.json/);
  assert.match(workflow, /actions\/runs\/\$PREFLIGHT_RUN_ID/);
  assert.match(workflow, /actions\/workflows\/release-preflight\.yml/);
  ...
  assert.match(workflow, /workflow_dispatch/);
  assert.match(workflow, /preflight-metadata\/preflight\.json/);
  assert.match(
    workflow,
    /publish:[\s\S]*?environment:\s*release[\s\S]*?name:\s*Revalidate the remote annotated tag[\s\S]*?name:\s*Create the GitHub Release/,
  );
});
...
test("the release source builds the current v1.4.0 artifact set", async (t) => {
  ...
  assert.deepEqual(result.files, [
    "SHA256SUMS",
    "openai-compatible-imagegen-codex-plugin-1.4.0.zip",
    "openai-compatible-imagegen-shared-python-sha256-1.4.0.json",
    "openai-compatible-imagegen-skill-1.4.0.zip",
  ]);
