...
3. To edit an image, call `edit_image` with the parent image ID. For transparent delivery, also pass top-level `transparency` and call `deliver_image` once after success. For an ordinary edit, call `render_image_results` once with the returned child ID before the final response. Do not call `get_image_artifact` only to display it.
