...
For API Key requests, `quality` accepts `auto`, `low`, `medium`, `high`, `xhigh`, and `max`. GPT Image 2.5 Sunburst and Flare support the new levels; other models and providers may support only a subset. Atlas still accepts only `low`, `medium`, and `high`. Preserve explicitly requested quality and report unsupported values without downgrading or switching models. ChatGPT host generation does not expose API model or quality selection. `list_image_models` returns `activeProfile` alongside `models`; resolve the active model by this ID, never by a fixed profile name or the first catalog entry.
