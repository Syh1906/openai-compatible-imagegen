...
Generation success remains separate from QA. `ok=true` means at least one complete API original was published; it does not claim semantic, aesthetic, dimensional, format, or transparency quality. A failed transparency, transform, or optional QA condition is reported with `delivery_ready=false` and warnings while the API image remains available. QA judges multi-image derivatives per image: passing derivatives are committed, while failed, unsupported, or not-evaluated images fall back to their published originals when that result can be assigned to one image. A global derivative count or global QA condition rolls back all staged derivatives. QA evidence never retains stale temporary paths.
