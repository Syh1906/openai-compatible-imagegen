...
| 使用 Plugin 时提示缺少 `dist/auth.json` | 可能误用了 Standalone 命令行入口 | 让 Codex 使用 Plugin 的 `inspect_image_config` 检查配置；Plugin 不使用安装目录中的 `auth.json` |
...
| 现象 | 含义 | 处理 |
| --- | --- | --- |
| 图片卡片提示“读取较慢，仍在等待” | 读取已保存的图片超过 8 秒，不代表生成失败 | 保持卡片打开，原读取请求完成后会自动显示图片，或报告实际读取错误；无需重新生成 |
| Provider 拒绝请求 | 配置的服务返回 API 错误 | 查看安全错误码和 provider 日志，不要自动切换路线 |
| 等待图片工具超时 | 等待结束，生成可能仍在继续 | 查询原 `jobId`；提交回执丢失时复用原提交键，不创建重复请求。见[任务恢复](./image-jobs.zh-CN.md) |
| 结果卡报告无效数据 | 产物 metadata 或字节未通过验证 | 保留原始错误，并检查已安装 Plugin 版本和构建身份 |
| 画布无法打开 | 产物、绑定或编辑器会话不可用 | 返回会话，从同一结果重新打开画布。仍然失败时，让 Codex 检查项目绑定和具体错误；刚更新 Plugin 时，完全退出并重启 Codex |
| 画布或图片预览展开后白屏，调整面板宽度后出现 | 可能与 Codex App 的面板显示刷新有关，参见[相关问题报告](https://github.com/openai/codex/issues/42694) | 轻拖右侧面板的分隔线调整宽度。这只能临时恢复显示；报告问题时请附上 Codex App 版本和复现步骤 |
| 切换任务后侧栏显示结果卡 | Codex 恢复了行内结果，而不是已打开画布 | 在同一卡片上选择 **继续编辑**；Plugin 会恢复保留的未发送草稿 |
| 未满足透明要求 | 原图成功，但透明检查未通过 | 保留原图并查看报告中的路线。[透明设置](./configuration.zh-CN.md#透明设置)说明了原生透明行为和本地处理限制 |
