...
| Plugin 使用中出现 `dist/auth.json` 缺失 | 是否误运行了 Standalone CLI | 使用 Plugin MCP 工具并通过 `inspect_image_config` 检查 Plugin 配置；不要在安装目录创建 `auth.json` |
...
| 画布或图片预览展开后白屏，改变面板宽度便立即显示 | 这类现象指向 Codex App 展开面板时的布局或可见状态更新；上游有[同类问题报告](https://github.com/openai/codex/issues/42694) | 临时恢复可轻拖右侧面板分隔线改变宽度，无需发送消息或重新生成图片。这不是永久修复；报告问题时附上 Codex App 版本和该复现步骤 |
