...
`url_download.proxy_mode` 默认使用环境代理。如果 provider 返回的图片 URL 通过该代理重复发生 TLS EOF，可以使用 `--allow-direct-url-download` 批准一次直连下载。只有在确认该 provider 的 URL 路线后，才把持久模式设为 `direct`。此设置只影响返回图片的下载，不影响图片 API 请求；运行时不会把 API key 发送到返回的 URL。
...
从已安装 Plugin 的 `skills/openai-compatible-imagegen/references/config.example.json` 开始配置。
...
项目文件不能替换活动 profile、provider、model、endpoint、认证来源、凭据环境变量、timeout、concurrency 或路线权限。不允许的覆盖会在网络请求前停止。
